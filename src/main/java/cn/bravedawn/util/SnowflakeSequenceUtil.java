...
import cn.bravedawn.config.SnowflakeSequenceConfig;
 ...
 */
@Component
@DependsOn("snowflakeSequenceConfig")
public class SnowflakeSequenceUtil {

    @Autowired
    private SnowflakeSequenceConfig config;
    ...
    public void init() {
        ...
        if (config.getWorkId() > maxWorkerId || config.getWorkId() < 0) {
            throw new IllegalArgumentException("机器ID不能大于31或者不能小于0");
        }

        if (config.getCenterId() > maxDataCenterId || config.getCenterId() < 0) {
            throw new IllegalArgumentException("数据中心ID不能大于31或者不能小于0");
        }

        this.workId = config.getWorkId();
        this.dataCenterId = config.getCenterId();
    }
